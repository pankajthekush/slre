...
def launch_chrome_development():
    ...
    subprocess.call('{0}/change_permission.sh'.format(currdir))
    if not run_status:
        subprocess.call('{0}/launch_chrome.sh'.format(currdir))
        print('Started bash script containing google-chrome run command')
        return "Started bash script containing google-chrome run command"
    else:
